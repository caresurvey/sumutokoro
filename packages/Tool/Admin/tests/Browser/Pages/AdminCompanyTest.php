...
class AdminCompanyTest extends DuskTestCase
{
    ...
    public function 法人管理テスト()
    {
        $this->browse(function (Browser $browser) {
            ...
            $browser->assertInputValue('#CompanyName', '追加法人変更');
            $browser->assertInputValue('#CompanyLongName', '追加法人フルネーム変更');
            $browser->assertInputValue('#CompanyKana', '追加ふりがな変更');
            ...
            $browser->assertInputValue('#CompanyZip1', '099');
            $browser->assertInputValue('#CompanyZip2', '1239');
            $browser->assertSelected('#CompanyPrefecture', '3');
            $browser->assertSelected('#CompanyCity', '3');
            $browser->assertInputValue('#CompanyAddress', '追加住所3変更');
            $browser->assertInputValue('#CompanyLat', '44.96281006321748');
            $browser->assertInputValue('#CompanyLng', '143.95817790725756');
            $browser->assertInputValue('#CompanyStart', '追加法人開始変更');
            $browser->assertInputValue('#CompanyPresident', '追加代表変更');
            $browser->assertInputValue('#CompanyHistory', '追加社歴変更');
            $browser->assertInputValue('#CompanyStaff', '追加従業員数変更');
            $browser->assertInputValue('#CompanyMsg', '追加備考変更');
