...
@section('content')
  ...
  <form action="{{asset('/')}}{{config('myapp.app_admin_prefix')}}/company/{{$data['company']['id']}}"
  ...
  <div class="bg-white shadow-sm rounded mb-5 p-4">
    <table class="w-full text-sm text-left text-gray-600">
      ...
      <tr class="bg-white">
        ...
        <td class="py-4 px-4 w-4/5">
          <p class="leading-6">
            {{date('Y/m/d H:i:s',  strtotime($data['company']['updated_at']))}}<br>
            {{$data['company']['user']['name']}}さん
          </p>
        </td>
      </tr>
      </tbody>
  ...
  <div class="bg-white shadow-sm rounded mb-10 p-4">
    <form action="{{asset('/')}}{{config('myapp.app_admin_prefix')}}/company/{{$data['company']['id']}}"
          onsubmit="return confirm('本当に{{$data['company']['name']}}を削除してもよろしいですか？（※元には戻せません）')" method="post"
          accept-charset="UTF-8" id="CompanyDeleteForm">
      @method('DELETE')
      @csrf
      <table class="w-full text-sm text-left text-gray-500">
        <tbody>
